...
const defaultOrganizationsData: OrganizationSeedData[] = [
  ...
  // COMRJ organization
  {
    ...
    sigla: "COMRJ",
    description: "Realiza a aquisição de materiais e serviços no mercado nacional, conforme atribuições designadas, apoiando as unidades navais e administrativas.",
    role: "COMRJ",
  },
  // Child organizations (DEPOSITO) under CSupAb
  {
    ...
    sigla: "DepFMRJ",
    description: "Responsável pelo recebimento, armazenamento, e distribuição de uniformes e fardamentos aos militares da Marinha do Brasil.",
    role: "DEPOSITO",
    parentSigla: "CSupAb",
  },
  {
    ...
    sigla: "DepSIMRJ",
    description: "Responsável pelo recebimento, armazenamento, e distribuição de gêneros alimentícios e materiais comuns, garantindo o apoio logístico às OMs da Marinha do Brasil.",
    role: "DEPOSITO",
    parentSigla: "CSupAb",
  },
  {
    ...
    sigla: "DepMSMRJ",
    description: "Responsável pelo recebimento, armazenamento, e distribuição de insumos, medicamentos e materiais médico-hospitalares às OMs da Marinha do Brasil",
    role: "DEPOSITO",
    parentSigla: "CSupAb",
  },
  // Child organizations (DEPOSITO) under COpAb
  {
    ...
    sigla: "DepSMRJ",
    description: "Responsável pelo recebimento, armazenamento, e distribuição de peças de reposição e sobressalentes às OMs da Marinha do Brasil",
    role: "DEPOSITO",
    parentSigla: "COpAb",
  },
  {
    ...
    sigla: "CMM",
    description: "Responsável pelo recebimento, armazenamento, e distribuição de munições e explosivos utilizados pelas OMs.",
    role: "DEPOSITO",
    parentSigla: "COpAb",
  },
  {
    ...
    sigla: "DepCMRJ",
    description: "Responsável pelo recebimento, estocagem, controle de qualidade e distribuição de combustíveis e lubrificantes para as OM da MB.",
    role: "DEPOSITO",
    parentSigla: "COpAb",
  },
];
...
export class OrganizationSeedModule implements SeedModule {
    ...
    for (const orgData of parentOrgs) {
      try {
        ...
        if (result.created) {
          ...
          skipped++;
          organizationIdMap.set(orgData.sigla, result.organization.id);
          this.logger.info(`Organization ${orgData.sigla} already exists, skipping`);
        }
      } catch (error) {
        this.logger.error(`Failed to create organization ${orgData.sigla}: ${error}`);
        throw error;
      }
    }
  ...
  private async createChildOrganizations(
    ...
    for (const orgData of childOrgs) {
      try {
        ...
        const parentId = await this.getParentOrganizationId(prisma, orgData.parentSigla || "");
  ...
  private async createOrganization(
    ...
    return { created: true, organization };
  }
